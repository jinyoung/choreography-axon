...
func (self *Deadline) Get(c echo.Context) error {
	...
	embeddedRel, _ := hal.NewResourceRelation("deadlines")
...
func (self *Deadline) FindById(c echo.Context) error{
	repository := DeadlineRepository()
...
func (self *Deadline) Persist(c echo.Context) error{
	repository := DeadlineRepository()
...
func (self *Deadline) Put(c echo.Context) error{
	repository := DeadlineRepository()
	id, _ := strconv.Atoi(c.Param("id"))
	params := make(map[string]string)
...
func (self *Deadline) Remove(c echo.Context) error{
	repository := DeadlineRepository()
	id, _ := strconv.Atoi(c.Param("id"))
	self, err := repository.FindById(id)
	if err != nil {
